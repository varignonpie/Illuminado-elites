...
class TransportApp:
    def __init__(self, root):
        ...
        # Load persistent data early so widgets reflect correct state when created
        self.load_state()
        self.load_history()
    ...
    def create_widgets(self):
        header = ttk.Label(self.root, text=f"{EMOJI_VARIGNON} Varignon Electronics Transport Options", font=FONT_HEADER, foreground=LABEL_COLOR)
        header.pack(pady=10)
        ...
        try:
            ...
            # theme may not be available on all platforms; ignore if not
            pass
    ...
    def buy_ticket(self):
        ...
        if confirm:
            ...
            if proceed:
                # Pass the booked departure so we record the actual booked time (before advancing)
                self.complete_purchase(name, booked_departure=departure)

    def complete_purchase(self, name, booked_departure=None):
        data = self.transport_data[name]
        seats = data['seats']
        if seats > 0:
            # decrement seats and advance schedule AFTER capturing booked departure
            data['seats'] -= 1
            # advance schedule index for the next available departure
            data['schedule_index'] = (data['schedule_index'] + 1) % len(data['schedule'])
            data['next_departure'] = data['schedule'][data['schedule_index']]
            self.refresh_treeview()
            entry = {
                'name': name,
                'departure': booked_departure if booked_departure is not None else data.get('next_departure'),
                'price': data['price'],
